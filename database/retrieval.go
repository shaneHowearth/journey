...
const stmtRetrievePostsCountByTag = "SELECT count(*) FROM posts, posts_tags WHERE posts_tags.postID = posts.id AND posts_tags.tagID = ? AND page = 0 AND status = 'published'"
...
const stmtRetrieveTags = "SELECT tagID FROM posts_tags WHERE postID = ?"
